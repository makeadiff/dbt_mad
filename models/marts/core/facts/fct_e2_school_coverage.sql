...
select
    ...
    max(slc.total_slots) as total_slots,
    max(clc.total_classes) as total_classes,
    ...
    max(vas.total_volunteers_assigned) as total_volunteers_assigned
...
left join volunteers_assigned vas
    on scs.chapter_id = vas.chapter_id
    and scs.academic_year = vas.academic_year
left join slot_counts slc
    on scs.chapter_id = slc.chapter_id
    and scs.academic_year = slc.academic_year
left join class_counts clc
    on scs.chapter_id = clc.chapter_id
    and scs.academic_year = clc.academic_year
left join {{ ref('dim_school_academic_year_status') }} st
    on scs.chapter_id = st.school_id::text
    and scs.academic_year = st.academic_year
group by scs.chapter_id, scs.academic_year, st.is_ay_active
